                        ...
                        {matchStatsSubTab === "aptitude" && (
                          <div>
                            {mPlayers.length === 0 ? (
                              ...
                              const norm = (id) => {
                                const ev = mEventList.find(e => e.id === id);
                                return ev ? Math.min((s[id] || 0) / (ev.max || 10), 1) : 0;
                              };
                              const scores = {
                                GK: Math.round(norm("cover") * 50 + norm("intercept") * 30 + norm("press") * 20),
                              ...
                              const posColor = POS_COLOR[p.position] || "#e8eaf0";
                              return (
                                <div key={p.key} style={{ background: "#0d1b2e", border: `1px solid ${noData ? "#1e3a5f" : POS_COLOR[recommended] + "55"}`, borderRadius: 12, padding: "14px 16px", marginBottom: 10 }}>
                                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                      <div style={{ fontSize: 22, fontWeight: 900, color: posColor }}>{p.number}</div>
                                      <div>
                                        <div style={{ fontSize: 14, fontWeight: 700 }}>{p.name}</div>
                                        <div style={{ fontSize: 10, color: "#4a7fa5" }}>登録: {p.position || "?"}</div>
                                      </div>
